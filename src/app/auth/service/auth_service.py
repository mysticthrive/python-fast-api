...
class AuthService:
    ...
    async def login(self, email: str, password: str) -> TokenBearer:
        user = await self.user_service.one(filters=[
    ...
    async def confirm_user(self, jwt: str) -> None:
        ...
        if token is None:
            raise UnprocessableEntityException(
                error_no=ErrorNo.CONFIRM_TOKEN_INVALID,
                message="Confirmation token is invalid"
            )

        if token.token_type != TokenType.CONFIRMATION:
            raise UnprocessableEntityException(
                error_no=ErrorNo.CONFIRM_TOKEN_TYPE_WRONG,
                message="Confirmation token is invalid"
            )
        user = await self.user_service.one(filters=[
        ...
        if user is None:
            raise UnprocessableEntityException(
                error_no=ErrorNo.CONFIRM_TOKEN_USER_NOT_FOUND,
                message="Confirmation token is invalid"
            )
        if user.session != token.session:
            raise UnprocessableEntityException(
                error_no=ErrorNo.CONFIRM_TOKEN_USER_SESSION_INVALID,
                message="Confirmation token is invalid"
            )
    ...
    async def refresh(self, jwt: str) -> TokenBearer:
        ...
        if token is None:
            raise UnauthorizedException(
                error_no=ErrorNo.REFRESH_TOKEN_INVALID,
                message="Unauthorized!"
            )
        if token.token_type != TokenType.REFRESH:
            raise UnauthorizedException(
                error_no=ErrorNo.REFRESH_TOKEN_TYPE_INVALID,
                message="Unauthorized!"
            )
        ...
        if user is None:
            raise UnauthorizedException(
                error_no=ErrorNo.REFRESH_TOKEN_USER_NOT_FOUND,
                message="Unauthorized!"
            )
        if user.status != UserStatus.ACTIVE:
            raise UnauthorizedException(
                error_no=ErrorNo.REFRESH_TOKEN_USER_STATUS_NOT_ACTIVE,
                message="Unauthorized!"
            )

        if user.session != token.session:
            raise UnauthorizedException(
                error_no=ErrorNo.REFRESH_TOKEN_USER_SESSION_INVALID,
                message="Unauthorized!"
            )
