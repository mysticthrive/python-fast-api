...
from src.app.user_notification.dto.user_notification import UserNotificationListRequest, UserNotificationCreateRequest
...
class UserNotificationController(BaseController):
    ...
    async def user_list(self, req: UserNotificationListRequest = Depends()) -> JsonApiResponse:
        notifications = await self.container.user_notification_service().all(
            filters=[
                Filter("user_id", FilterOperator.EQ, 1),
